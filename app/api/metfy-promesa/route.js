export async function POST(request) {
  try {
    ...
    const facturasResponse = await fetch('https://cliente.metfy.cl/api/v1/GetInvoices', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        token,
    ...
    const promesaResponse = await fetch('https://cliente.metfy.cl/api/v1/PromesaPago', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        token,
